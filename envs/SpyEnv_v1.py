...
class SpyEnv_v1(Env):
  ...
  def moveOpponentAgent(self, agentNum):
    ...
    agentAirportIndex = self.getAirPortIndex(self.state[agentNum])
    spyAirportIndex = self.getAirPortIndex(self.state['spyPosition'])
    path = self.shortest_path(agentAirportIndex, spyAirportIndex)

    #get new grid position for agent
    newRow, newCol = self.getRowAndColumn(path[1])
    #set agent new state position
    self.state[agentNum] = [newRow, newCol]
